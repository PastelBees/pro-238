...
#function so user can login to account
def login():
    #requires the user to type in both the email and password, or they will get an error
    try:
        ...
        #if the user email and password is correct, they will be able to login
        if user:
            session["email"] = email
        ...
        #if the user email and password is wrong, they won't be able to login and will get an error
        else:
            return jsonify({
                "status": "error",
                "message": "Not sure"
...
#when the user logs out, it removes their email and password and essentially resets kind of (?)
def logout():
    try:
        session["email"] = None
...
#getting and storing(?) the user info that is inputed into the app to add their address
def add_address():
    try:
...
#getting the product info that the user chose and user id and email to create the order
def create_order():
    try:
        user_email = session.get("email")
        user_query = f"select * from users where email='{user_email}';"
...
#function for the user to request for help/send an attachment
def submit_help():
...
#function for the user to be able to send the file/attachment in the help area
def download(filename):
    return send_file(os.path.join(UPLOAD_FOLDER, filename), as_attachment=True)

@api.route("/search-order")

#function for the user to search for the orders they made
def search_order():
...
#i'm not really sure about this one, but i think it makes this api run?
def execute():
    try:
